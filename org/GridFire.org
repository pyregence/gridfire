...
an existing burn parameter (raster).
...
To initialize an existing burn parameter from a raster we have two
options. We can read rasters from a Postgresql database or a raster
file on disk.

To initialize a burn parameter from a raster in our PostGIS database
