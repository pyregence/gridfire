...
{:paths ["src" "resources"]
 ...
 :deps {org.clojure/clojure                      {:mvn/version "1.10.1"}
        ...
        ;; TODO Point to Magellan in clojars
        ...
        org.springframework/spring-core          {:mvn/version "5.2.8.RELEASE"}}
...
#+begin_src sh :results silent :exports code :tangle ../resources/import-rasters.sh :padline no :no-expand :comments link
#!/bin/sh
...
*Note:* You must add any spatial reference systems used by the rasters
you intend to import into the database *before* running raster2pgsql.

If there is a need to update the srid of your layers you can use the following script.

*Note:* This script needs to be run in the same folder as where these rasters reside.

#+begin_src sh :results silent :exports code :tangle ../resources/update-landfire-srids.sh :padline no :no-expand :comments link
#!/bin/sh
...
The configuration file can be broken up into 3 sections as described below:
...
enabled database or we can read raster files from disk. This behavior is
controlled by Section 1 of the configuration file.

Here is a complete sample configuration for using layers from our postigs enabled
database.
...
#+begin_src clojure :results silent :exports code :tangle ../resources/sample_postgis_config.edn :padline no :no-expand :comments link
{
 ...
 :db-spec                     {:classname   "org.postgresql.Driver"
                               ...
                               :user        "gridfire"}
 :layer->table                {:elevation          "landfire.dem WHERE rid=100"
...
Here is a complete sample config for using layers from disk.
...
{
;; Section 1: Landscape data to be shared by all simulations
 :layer-files                 {:aspect             "test/gridfire/resources/asp.tif"
