...
If there is a need to update the srid of our layers we can use the
following script.
...
To run the script, give it the database name, schema name and srid we
wish the layers to have.
#+begin_src bash
...
Then we can use the following script to import ignition rasters into
...
To run the script, give it the username name, schema name and srid we wish the layers to have.
#+begin_src bash
sh import_ignition_rasters.sh gridfire ignition
#+end_src
...
1. Postgresql (an open source database) with the PostGIS spatial
   extensions. This interface is described in Section [[PostGIS Bridge]].

2. Magellan A Clojure library for interacting with GeoTools, an open
   source Java library that provides tools for geospatial data. This
   interface is described in Section [[Magellan]].
...
called *geotiff-raster-to-matrix*, which given a filepath to the raster
file, constructs the same map of information as
...
To fetch landfire layers from a Postgersql database include the
following mappings:
- *fetch-layer-method*: a keyword to denote which method we want to use
- *db-spec*: a map of database connection information to our Postgresql
  database.
...
an existing burn paremeter (raster).
...
3. For a range of values, a range of values, a value from the range
   [inclusive exclusive] will be randomly selected in each simulation
...
To initialize an existing burn paremeter from a raster we have two
options. We can read rasters from a Postgresql database or a raster
file on disk.

To initialize a burn paremeter from a raster in our PostGIS database
...
For all options in this section, you may enter values in one of the
...
our postigs enabled database and initializing burn points from a range
of values.
...
Here is a complete sample config for reading landfire-layers from disk
and initializing a burn paremeter from reading a raster file from
disk.
...
* raster2pgsql-import-ignition-rater
