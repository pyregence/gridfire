...
{:fetch-ignition-method :postgis
 :ignition-layers       "ignition.ign WHERE rid=1"}
#+end_src
...
{:fetch-ignition-method :geotiff
 :ignition-layers       "test/gridfire/resources/ign.tif"}
#+end_src
...
from rasters. To use weather data from raster we have two options. We
can read rasters from a PostGIS database or a raster file on disk.

To use weather raster in our PostGIS database include the following
mappings: 
...
our PostGIS-enabled database and initializing burn points from a range
of values.
