...
by setting *:spatial-type* to *:pixel*.
...
with "white noise", without one independent random variable for grid cell ('pixel').
...
which can contain billions of cells, of only a few millions will actually use their perturbation
during simulation.

One strategy we previously used is laziness: sampling the per-pixel random variables
as they are requested by the simulation, while maintaining a cache of the samples values,
...
HDPP apprixmates the \text{p}: \text{cell} \mapsto \text{perturbation} random function
by a composition of:
...
creating more correlations than true IID randomness, but we have already have accepted a similar tradeoff
...
To specifiy a different period you must include a `:burn-period` key at
the top level of the config file. The spec for the `:burn-period`
config is as follows:

- *:start* : a string with the format HH:MM
- *:end* : a string with the format HH:MM
- *:weather-data-start-timestamp* : an instant timestamp
