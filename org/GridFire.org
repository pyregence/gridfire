...
#+DATE: Copyright 2011-2021 Spatial Informatics Group, LLC
#+OPTIONS: ^:{} toc:nil h:3
#+LATEX_CLASS: article
...
2.0 (EPLv2) https://www.eclipse.org/legal/epl-2.0/. See LICENSE.txt in
the top level directory of this repository for details. Please contact
Gary Johnson (gjohnson@sig-gis.com), David Saah (dsaah@sig-gis.com),
Max Moritz (mmoritz@sig-gis.com), or Kenneth Cheung
(kcheung@sig-gis.com) for further information about this software.
...
Before we run tests, we'll need to set up a test database and import
rasters into it. We will be prompted for the postgres and
gridfire_test user's password. The postgres user's password will be
whatever it is when we set up Postgres. For the gridfire_test user's
...
tags to add any additional information that was not available from
...
*** Server Configuration
 ...
 1. software-dir: The directory the gridfire repo is cloned into.

 2. incoming-dir: The directory where the server will look for
 ...
 3. active-fire-dir: The directory where the server will look for
 ...
 4. data-dir: The directory into which input deck tarballs from
    incoming-dir are unpacked. The server keeps the latest 20 unpacked
    tarballs. (see cleanup.sh in this directory)

 5. log-dir: The directory into which log files are written. The server
