...
The pre-processing script at `resources/burn_period_from_sunrise_sunset.clj`
may be used to transform GridFire configuration files so that the `:burn-period`
is resolved from (approximate) sunrise and sunset hours.
