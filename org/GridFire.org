...
# test/.../elm_to_grid_test.clj
> 28: At what point are you using "test/gridfire/resources/config_test/outputs"?
> It doesn't look like this directory is ever used.

- This is needed when running `s/valid?`. The output directory must be a readable directory.

# Regarding suppression-coefficient vs suppression-curve-sharpness
for the sake of time, I'll revert back to the `:supression-coefficient` key and
add a backlog task to deprecate that key for the `:suppress-curve-sharpness`.
                                                     :suppression-coefficient])
