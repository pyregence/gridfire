...
#+begin_src sh :results silent :exports code
java -jar gridfire-1.2.0.jar myconfig.edn
#+end_src
...
#+begin_src clojure :results silent :exports code :tangle ../resources/sample_config.edn :padline no
{
 ...
 :simulations                 10
 :ignition-site               ([25 40] [10 10] [30 60]) ;; point(s) represented as [row col]
 :ignition-site               {:min-row 10 :max-row 90
                               :min-col 20 :max-col 80} ;; alternative option: randomly sample points from a range
 :max-runtime                 60.0   ;; double (minutes)
 :wind-speed-20ft             (10.0 20.0)   ;; double (miles/hour)
 :wind-from-direction         (0.0 90.0 180.0 270.0)   ;; double (degrees clockwise from north)
 ...
 ;; Section 3: Types and names of outputs
