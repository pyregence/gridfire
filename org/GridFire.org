...
GridFire supports suppresssion of fires. Suppression emulates human
...
GridFire supports two suppresssion algorithms. The to the *:suppression*
...
- *suppression-dt*: the time (in minutes) betweeen suppresssion events.
- *suppression-curve-calibration-coefficient*: a constant calibration unit

#+begin_src clojure
{:suppression {:suppression-dt                            60.0
               :suppression-curve-calibration-coefficient 2.0}}
#+end_src

Another approach uses the suppression difficulty index raster layer.
To enble this include the following:
