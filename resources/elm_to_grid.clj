...
(defn merge-override-config
  "Essentially equivalent to (update options :output-edn merge override-config),
  except that nil values in override-config will cause a dissoc."
  [override-config-file-path options]
  (update options
          :output-edn
          (fn [output-edn]
            (let [override-config (some-> override-config-file-path
                                          (slurp)
                                          (edn/read-string))]
              (reduce-kv (fn override-kv [out-edn k v]
                           (if (nil? v)
                             (dissoc out-edn k)
                             (assoc out-edn k v)))
                         output-edn
  ...
  (if elmfire-summary-csv
    (with-open [reader (io/reader elmfire-summary-csv)]
      (let [[_header-row & data-rows] (doall (csv/read-csv reader))]
        (cond->> data-rows
          every-nth-elmfire (take-nth every-nth-elmfire)
          :always           (map #(parse-fire-stats-line elmfire-config %))
          :always           (assoc options :elmfire-summary-maps))))
    options))
