...
(defn- build-intrange-mapping
  [elmfire-config key L+H default-val]
  (let [incomplete-intrange->v (some->> (filterv (fn by-key [[k _]] (str/includes? k key)) elmfire-config)
                                        (seq)
...
(test/deftest build-intrange-mapping-test
    ...
    (test/testing "multiple lines"
      (let [config {"SOME_PARAM(0:256)" 1200.0
        ...
        (test/is (= {[0 256]   1200.0
                     [257 303] 100.0}
                    (build-intrange-mapping config "SOME_PARAM" [1 303] 1.0))))))
    ...
    (test/testing "single line"
      (let [config {"SOME_PARAM(2:8)" [5.0 5.0 6.0 6.0 7.0 7.0 8.0 8.0]}]
        (test/is (= {[1 1]  1.0
                     ...
                     [8 8]  8.0
                     [9 10] 1.0}
                    (build-intrange-mapping config "SOME_PARAM" [1 10] 1.0)))))
...
(defn extract-global-surface-spotting-percents
  ...
  (cond
    ...
    :else (build-intrange-mapping elmfire-config "SURFACE_FIRE_SPOTTING_PERCENT(" [1 303] 0.0)))
      ...
      (-> (assoc-in [:spotting :surface-fire-spotting]
                    {:spotting-percent             (extract-global-surface-spotting-percents elmfire-config)
                     :critical-fire-line-intensity (or (some-> CRITICAL_SPOTTING_FIRELINE_INTENSITY kW-m->Btu-ft-s)
                                                       (build-intrange-mapping elmfire-config "CRITICAL_SPOTTING_FIRELINE_INTENSITY(" [1 303] 0.0)
                                                       0.0)})
          (apply-spotting-multipliers (build-intrange-mapping elmfire-config "SURFACE_FIRE_SPOTTING_PERCENT_MULT" [1 303] 1.0))))))
