...
(defn- resolve-suppression [suppression-params i]
  (when suppression-params
    (reduce-kv (fn [acc key val]
                 (assoc acc key (if (vector? val)
                                  (get val i)
                                  val)))
               {}
               suppression-params)))
...
(defrecord SimulationInputs
    [^long num-rows
     ...
     spotting
     suppression
     fuel-model->spread-rate-adjustment])
  ...
  [^long i
   {:keys
    [num-rows num-cols grass-suppression? output-csvs? envelope ignition-matrix cell-size max-runtime-samples
     ...
     output-flame-length-max output-layers fuel-model->spread-rate-adjustments]
    :as inputs}]
  (tufte/profile
   {:id :run-simulation}
   (let [rand-gen           (if random-seed (Random. (+ ^long random-seed i)) (Random.))
         simulation-inputs  {:num-rows                           num-rows
                             ...
                             :spotting                           spotting
                             :suppression                        (resolve-suppression suppression i)
                             :fuel-model->spread-rate-adjustment (when fuel-model->spread-rate-adjustments
                                                                   (fuel-model->spread-rate-adjustments i))}
