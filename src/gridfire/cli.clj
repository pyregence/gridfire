  ...
  (doseq [config-file config-files]
    (let [config           (edn/read-string (slurp config-file))
      ...
      (run-simulations
       ...
       (n-cycle simulations (:max-runtime config))
       (n-cycle simulations (:wind-speed-20ft config))
       (n-cycle simulations (:wind-from-direction config))
       (n-cycle simulations (:fuel-moisture config))
       (n-cycle simulations (:foliar-moisture config))
       (n-cycle simulations (:ellipse-adjustment-factor config))
