...
(defn postprocess-simulation-results
  [wrf-cell-id lon lat output-directory results-table]
...
;; Need to nil-proof this function
  ...
  (let [landfire-cell-size        98.425 ;; ft
    ...
    (->> (read-wrf-cells-list wrf-cells-file start end)
         ...
         (r/map (fn [{:keys [wrf_cell_id lon lat lw_moisture]}]
                  (let [weather-readings (fetch-extreme-weather-readings db-spec wrf_cell_id)
                        landfire-data    (fetch-landfire-data db-spec wrf_cell_id)
                        ignition-sites   (into []
                                               (comp (distinct) (take 1000))
                                               (repeatedly #(mop/+ [66 66] (random-cell 67 67))))]
                    (->> (run-monte-carlo-fire-spread landfire-data landfire-cell-size ignition-sites weather-readings lw_moisture
                         ...
                         (postprocess-simulation-results wrf_cell_id lon lat output-directory)))))
         ;;(into [])
         (r/fold 25 r/cat r/append!)
...
(comment
  ;; Run this on iwap03. Then scp this file to iwap04 and iwap05.
