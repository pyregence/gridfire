...
(defn add-aggregate-matrices
  [{:keys
    [max-runtime-samples num-rows num-cols output-burn-count? output-burn-probability output-flame-length-sum?
     output-flame-length-sum output-flame-length-max? output-flame-length-max output-spot-count?] :as inputs}]
  (assoc inputs
         ...
         :flame-length-sum-matrix (when (or output-flame-length-sum
                                            output-flame-length-sum?) (t/new-tensor [num-rows num-cols]))
         :flame-length-max-matrix (when (or output-flame-length-max
                                            output-flame-length-max?) (t/new-tensor [num-rows num-cols]))
