...
(defn add-input-layers
  [config]
  (let [future-or-layer                      (fn [f] (if (future? f) @f f))
    ...
    (assoc config
           :envelope                             (future-or-layer envelope)
           :aspect-matrix                        (future-or-layer aspect-matrix)
           :canopy-base-height-matrix            (future-or-layer canopy-base-height-matrix)
           :canopy-cover-matrix                  (future-or-layer canopy-cover-matrix)
           :canopy-height-matrix                 (future-or-layer canopy-height-matrix)
           :crown-bulk-density-matrix            (future-or-layer crown-bulk-density-matrix)
           :elevation-matrix                     (future-or-layer elevation-matrix)
           :fuel-model-matrix                    (future-or-layer fuel-model-matrix)
           :slope-matrix                         (future-or-layer slope-matrix)
           :ignition-matrix                      (future-or-layer ignition-matrix)
           :ignition-mask-matrix                 (future-or-layer ignition-mask-matrix)
           :temperature-matrix                   (future-or-layer temperature-matrix)
           :relative-humidity-matrix             (future-or-layer relative-humidity-matrix)
           :wind-speed-20ft-matrix               (future-or-layer wind-speed-20ft-matrix)
           :wind-from-direction-matrix           (future-or-layer wind-from-direction-matrix)
           :fuel-moisture-dead-1hr-matrix        (future-or-layer fuel-moisture-dead-1hr-matrix)
           :fuel-moisture-dead-10hr-matrix       (future-or-layer fuel-moisture-dead-10hr-matrix)
           :fuel-moisture-dead-100hr-matrix      (future-or-layer fuel-moisture-dead-100hr-matrix)
           :fuel-moisture-live-herbaceous-matrix (future-or-layer fuel-moisture-live-herbaceous-matrix)
           :fuel-moisture-live-woody-matrix      (future-or-layer fuel-moisture-live-woody-matrix))))
